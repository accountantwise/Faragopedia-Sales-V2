...
7. ~~Switch ingest to a cheaper/faster model than Sonnet.~~ `INGEST_AI_PROVIDER`/`INGEST_AI_MODEL` env var overrides now exist (ADR 0004) — still unset in `.env`, so ingest stays on Sonnet. Evaluate a cheaper model (e.g. Haiku) for extraction quality vs. cost/speed before setting them.
