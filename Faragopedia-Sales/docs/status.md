...
| ---------- | ------------ | --------------------------------------------- |
| 2026-07-01 | Claude | **External API exposure & auth — backend middleware done** (branch: `feature/external-api-auth`). A Trigger.dev automation in the separate `FaragoProjects-Rocketreach-leads-research` project needs to call `POST /api/paste` + `POST /api/sources/{filename}/ingest` from off-network. Added `external_api_key_middleware` to `main.py`: requires `X-API-Key` matching `FARAGOPEDIA_API_KEY` on any `/api/*` request carrying Cloudflare's `CF-Connecting-IP` header (absent on internal Docker-network calls, so frontend traffic is unaffected). No-op when `FARAGOPEDIA_API_KEY` is unset. `FARAGOPEDIA_API_KEY` wired into `.env.example` and `docker-compose.yml`. ADR 0003 written. 6 new tests passing (`test_external_api_key_middleware.py`); 256 pre-existing tests unaffected (11 pre-existing failures confirmed unrelated — Python 3.14 env issues in `test_api.py`/`test_setup_routes.py`/`test_wiki_import.py`, present on `main` before this branch). This is the defense-in-depth layer; the primary gate (Cloudflare Access + Service Token on a new `api.faragopedia.ai-wise.uk` tunnel route) is dashboard-only and still needs to be configured by the user — see ADR 0003 and the handoff doc in `FaragoProjects-Rocketreach-leads-research/docs/`. **Next:** user configures the Cloudflare Tunnel route + Access app + Service Token, sets `FARAGOPEDIA_API_KEY` in the Portainer stack env, redeploys, then sends `FARAGOPEDIA_API_URL` / `FARAGOPEDIA_CF_CLIENT_ID` / `FARAGOPEDIA_CF_CLIENT_SECRET` / `FARAGOPEDIA_API_KEY` back to the Trigger.dev project. |
